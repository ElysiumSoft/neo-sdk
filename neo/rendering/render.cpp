...
/// <returns>array of ImVec2 objects</returns>
...
void render::draw_box(float x1, float y1, float x2, float y2, color clr, float size) {/*
	...
	g::draw_list->AddQuad(ImVec2(x1, y1), ImVec2(x2, y1), ImVec2(x2, y2), ImVec2(x1, y2), utils::to_im32(clr, clr.a()), size);
}
...
void render::draw_text_ex(std::string text, const ImVec2& position, color col, const ImFont* font, float font_size) {
	g::draw_list->AddText(font, font_size, position, utils::to_im32(col, col.a()), text.c_str(), NULL, 0.0f, NULL);
}
